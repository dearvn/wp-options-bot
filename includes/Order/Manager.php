...
class Manager {
    ...
    public function all( array $args = [] ) {
        ...
        if ( ! empty( $args['symbol'] ) ) {
            global $wpdb;
            $symbol = sanitize_text_field( wp_unslash( $args['symbol'] ) );
            $args['where'][] = $wpdb->prepare( ' symbol = %s', $symbol );
        }
    ...
    public function create( $data ) {
        ...
        if ( ! $order_id ) {
            return new \WP_Error( 'optionsbot_order_create_failed', __( 'Failed to create order.', 'optionsbot' ) );
        }
         ...
         */
        do_action( 'optionsbot_order_created', $order_id, $order_data );
    ...
    public function update( array $data, int $order_id ) {
        ...
        if ( ! $updated ) {
            return new \WP_Error( 'optionsbot_order_update_failed', __( 'Failed to update order.', 'orderbot' ) );
        }
        ...
        if ( $updated >= 0 ) {
             ...
             * @param array $order_data
             */
            do_action( 'optionsbot_order_updated', $order_id, $order_data );

            return $order_id;
        }

        return new \WP_Error( 'optionsbot_order_update_failed', __( 'Failed to update the order.', 'optionsbot' ) );
    }
    ...
    public function delete( $order_ids ) {
        ...
        try {
            ...
            foreach ( $order_ids as $order_id ) {
                 ...
                 * @param int $order_id
                 */
                do_action( 'optionsbot_order_deleted', $order_id );
            }
            ...
            return new \WP_Error( 'optionsbot-order-delete-error', $e->getMessage() );
        }
    }
}
